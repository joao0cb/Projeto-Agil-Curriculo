...
export type StatusVariant = keyof NonNullable<Parameters<typeof variant>[0]>["variants"];
...
export function StatusBadge({ value, variant: forced }: StatusBadgeProps) {
  const mapped = forced ?? statusMap[value.toLowerCase().trim()] ?? "pending";
  return (
    <span className={twMerge(clsx(variant(mapped)))}>
